...
from pytractions.base import TList, Arg, NullPort, Port, Traction
...
class FilterTags(Traction):
    ...
    def _run(self, on_update=None) -> None:
        ...
        for tag in self.i_tags:
            if self.a_skip_sigs and (tag.name.endswith(".sig") or tag.name.endswith(".att") or tag.name.endswith(".sbom")):
                continue
...
class GetLastRepoTags(Tractor):
    ...
    a_executor: Union[ProcessPoolExecutor, ThreadPoolExecutor, LoopExecutor] = ThreadPoolExecutor(pool_size=5)
